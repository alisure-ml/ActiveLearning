...
class WN_Conv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, train_scale=False, init_stdv=1.0):
...
class WN_ConvTranspose2d(nn.ConvTranspose2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, output_padding=0, groups=1, bias=True, train_scale=False, init_stdv=1.0):
    ...
    def forward(self, input, output_size=None):
        ...
        output_padding = self._output_padding(input, output_size, stride=self.stride, padding=self.padding,
                                              kernel_size=norm_weight.shape[2:])
...
class Generator(nn.Module):
    ...
    def __init__(self, image_size, noise_size=100, num_label=10, large=False):
        ...
        if not large:
            self.core_net = nn.Sequential(
                nn.Linear(self.noise_size + self.num_label, 4 * 4 * 512, bias=False), nn.BatchNorm1d(4 * 4 * 512),
                nn.ReLU(),
            ...
            self.core_net = nn.Sequential(
                nn.Linear(self.noise_size + self.num_label, 2 * 2 * 1024, bias=False), nn.BatchNorm1d(2 * 2 * 1024),
                nn.ReLU(),
    ...
    def forward(self, noise, label):
        if label.dim() == 1:
            label = torch.nn.functional.one_hot(label.to(torch.int64), num_classes=self.num_label).to(
                torch.float32).to(noise.device).view(-1, self.num_label)
        else:
            label = label.to(torch.float32).to(noise.device).view(-1, self.num_label)
...
class Encoder(nn.Module):
    ...
    def forward(self, image, label):
        if label.dim() == 1:
            label = torch.zeros_like(label).to(torch.float32).view(-1, 1, 1, 1).repeat(1, 1, 32, 32).to(image.device)
        else:
            label = torch.argmax(label, dim=1) / self.num_label
            label = label.to(torch.float32).view(-1, 1, 1, 1).repeat(1, 1, 32, 32).to(image.device)
