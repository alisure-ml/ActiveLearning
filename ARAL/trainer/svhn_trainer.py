...
from data.data import Utils, DataLoader, get_svhn_loaders
...
class Trainer(nn.Module):
    ...
    def __init__(self, config, args):
        ...
        self.dis = svhn_model.Discriminative(config).cuda()
        self.gen = svhn_model.Generator(image_size=config.image_size, noise_size=config.noise_size).cuda()
        self.enc = svhn_model.Encoder(image_size=config.image_size, noise_size=config.noise_size, output_params=True).cuda()
        self.smp = svhn_model.Sampler(noise_size=config.noise_size).cuda()
        ...
        self.labeled_loader, self.unlabeled_loader, self.unlabeled_loader2, self.dev_loader = data.get_svhn_loaders(config)
    ...
    def train(self):
        ...
        while True:
            ...
            if iter % batch_per_epoch == 0:
                ...
                self.smp_optimizer.param_groups[0]['lr'] = max(min_lr, config.smp_lr * min(3. * (1. - epoch_ratio), 1.))
