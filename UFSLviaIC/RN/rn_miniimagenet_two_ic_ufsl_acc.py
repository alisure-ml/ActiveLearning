...
class Runner(object):
    ...
    def train(self):
        ...
        for epoch in range(Config.train_epoch):
            ...
            ###########################################################################
            # print
            Tools.print("{:6} loss:{:.3f} fsl:{:.3f} ic:{:.3f} ok:{:.3f}({}/{}) lr:{}".format(
                epoch + 1, all_loss / len(self.task_train_loader), all_loss_fsl / len(self.task_train_loader),
                all_loss_ic / len(self.task_train_loader), int(is_ok_acc) / int(is_ok_total),
                is_ok_acc, is_ok_total, self.feature_encoder_scheduler.get_last_lr()))
...
class Config(object):
    ...
    fe_dir = Tools.new_dir("../models/two_ic_ufsl_acc/{}_fe_{}way_{}shot.pkl".format(model_name, num_way, num_shot))
    rn_dir = Tools.new_dir("../models/two_ic_ufsl_acc/{}_rn_{}way_{}shot.pkl".format(model_name, num_way, num_shot))
    ic_dir = Tools.new_dir("../models/two_ic_ufsl_acc/{}_ic_{}way_{}shot.pkl".format(model_name, num_way, num_shot))
    pass
