...
class DatasetIC(Dataset):
    ...
    def __init__(self, data_list, transform, data_image=None, load_data=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.load_data:
            image_id = int(os.path.basename(image_filename).split(".png")[0])
...
class ICTestTool(object):
    ...
    def __init__(self, feature_encoder, ic_model, data_root, transform,
                 batch_size=64, num_workers=8, ic_out_dim=512, k=100, load_data=False):
        ...
        self.load_data = load_data
        if self.load_data:
            Tools.print("Load train_images_png to memory....")
            self.data_train_image = np.load(os.path.join(data_root, "train_images_png.npz"))["images"]
            Tools.print("Load val_images_png to memory....")
            self.data_val_image = np.load(os.path.join(data_root, "val_images_png.npz"))["images"]
            Tools.print("Load test_images_png to memory....")
            self.data_test_image = np.load(os.path.join(data_root, "test_images_png.npz"))["images"]
            pass

        self.train_loader = DataLoader(DatasetIC(self.data_train, transform=transform,
                                                 data_image=self.data_train_image, load_data=self.load_data),
                                       batch_size, False, num_workers=num_workers)
        self.val_loader = DataLoader(DatasetIC(self.data_val, transform=transform,
                                                 data_image=self.data_val_image, load_data=self.load_data),
                                     batch_size, False, num_workers=num_workers)
        self.test_loader = DataLoader(DatasetIC(self.data_test, transform=transform,
                                                 data_image=self.data_test_image, load_data=self.load_data),
                                      batch_size, False, num_workers=num_workers)
        pass
