...
class Runner(object):
    ...
    def load_model(self):
        if os.path.exists(Config.feature_encoder_dir):
            self.feature_encoder.load_state_dict(torch.load(Config.feature_encoder_dir))
            Tools.print("load feature encoder success from {}".format(Config.feature_encoder_dir))

        if os.path.exists(Config.relation_network_dir):
            self.relation_network.load_state_dict(torch.load(Config.relation_network_dir))
            Tools.print("load relation network success from {}".format(Config.relation_network_dir))
        pass
    ...
    def train(self):
        ...
        for episode in range(Config.train_episode):
            ...
            if (episode + 1) % Config.val_freq == 0:
                Tools.print()
                Tools.print("Val...")
                train_accuracy = self.val_train(episode)
                val_accuracy = self.val_val(episode)

                if val_accuracy > self.best_accuracy:
                    self.best_accuracy = val_accuracy
                    torch.save(self.feature_encoder.state_dict(), Config.feature_encoder_dir)
                    torch.save(self.relation_network.state_dict(), Config.relation_network_dir)
...
class Config(object):
    ...
    feature_encoder_dir = Tools.new_dir("../models/{}_fe_{}way_{}shot.pkl".format(model_name, num_way, num_shot))
    relation_network_dir = Tools.new_dir("../models/{}_rn_{}way_{}shot.pkl".format(model_name, num_way, num_shot))
